...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("nextProxy").addEventListener("click", async () => {
        try {
            ...
            const response = await fetch("http://localhost:3001/getProxy");
    ...
    btn4.onclick = async () => {
        const mt = await fetch("http://localhost:3001/saveMail");
        const data = await mt.json();
        if (data.error) {
            showAlert(data.error);
            return;
    ...
    const mt = await fetch("http://localhost:3001/get")
    const data = await mt.json()
    currentData = data;
    updateInfo();
    ...
    {
        const mt = await fetch("http://localhost:3001/getLink_NonEx")
        const data = await mt.json()
        if (data && !data.error) {
            ...
            const requestGetErrorEmail1 = await fetch(`http://localhost:3001/getError/${firstLink.email}`)
            ...
            const requestGetErrorEmail2 = await fetch(`http://localhost:3001/getError/${secondLink.email}`)
            ...
            buttonSendErrorEmail1.onclick = async () => {
                const requestSendErrorEmail1 = await fetch(`http://localhost:3001/setError/${firstLink.email}/${document.getElementById("email1").value}`)
            ...
            buttonSendErrorEmail2.onclick = async () => {
                const requestSendErrorEmail2 = await fetch(`http://localhost:3001/setError/${secondLink.email}/${document.getElementById("email2").value}`)
    ...
    buttonUndoLink.onclick = async () => {
        const requestUndoLink = await fetch("http://localhost:3001/undoLink")
    ...
    buttonNextLink.onclick = async () => {
        const requestNextLink = await fetch("http://localhost:3001/nextLink")
